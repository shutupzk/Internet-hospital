...
/*
var sig = new Sig({
sdk_appid : '1400013150',
account_type : 6788,
private_key : projectDir+'/key/private_key',
public_key : projectDir+'/key/public_key',
});
*/
var Sig = function(config) {
  ...
  // this.private_key = fs.readFileSync(path.join(config.private_key)).toString();
  // this.public_key = fs.readFileSync(path.join(config.public_key)).toString();
...
Sig.prototype._genSignContent = function(obj) {
  var arr = ['TLS.appid_at_3rd', 'TLS.account_type', 'TLS.identifier', 'TLS.sdk_appid', 'TLS.time', 'TLS.expire_after']
...
Sig.prototype.getSig = function(identifier) {
  ...
  obj['TLS.sig'] = Buffer.from(signature).toString('base64')

  var text = JSON.stringify(obj)
  var compressed = zlib.deflateSync(Buffer.from(text))
...
Sig.prototype.verifySig = function(sig, identifier) {
  try {
    ...
    var signature = Buffer.from(json['TLS.sig'], 'base64')
